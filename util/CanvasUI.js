...
class CanvasUI {
    constructor(content, config) {
        ...
        Object.entries(this.config).forEach(([name, value]) => {
            if (
                ...
                if (pos.left !== undefined && pos.x === undefined)
                    pos.x = pos.left;
                if (pos.top !== undefined && pos.y === undefined)
                    pos.y = pos.top;
                ...
                if (elm.type == "button") {
                    ...
                    if (self.keyboard) {
                        if (self.keyboard.visible) {
                            ...
                            let name;
                            Object.entries(self.config).forEach(
                                ([prop, value]) => {
    ...
    setClip(elm) {
        ...
        if (elm.clipPath !== undefined) {
            ...
            const pos =
                elm.position !== undefined ? elm.position : { x: 0, y: 0 };
        ...
        Object.entries(this.content).forEach(([name, content]) => {
            const config =
                self.config[name] !== undefined
                    ...
                    : self.config.body;
            const display =
                config.display !== undefined ? config.display : "block";
...
export { CanvasUI, CanvasKeyboard };
